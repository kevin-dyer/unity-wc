...
class UnityTable extends LitElement {
  ...
  async handleLowerThreshold(e) {
    let dataLength = this._flattenedData.length
    ...
    if ((nextOffset + this._visibleRowCount) >= dataLength && typeof this.onEndReached === 'function') {
      ...
      dataLength = this._flattenedData.length
      setTimeout(() => {
    ...
    let maxOffset = Math.max((dataLength - this._visibleRowCount), 0)
  ...
  _renderRow(row={}) {
    ...
    return html`
      <tr
        ...
        style="${rowStyle}"
      >
      <td class='centered select-row-wrapper'>
        <unity-checkbox
          class="select-row-checkbox"
