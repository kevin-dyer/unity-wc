...
// === CONSTANTS ===
// Box Types
const BOX_TYPE_FIXED = "fixed"
const BOX_TYPE_LABEL = "label"
const BOX_TYPE_SEARCH = "search"
const BOX_TYPE_PRIMARY = "button-primary"
const BOX_TYPE_SECONDARY = "button-secondary"
const BOX_TYPE_BORDERLESS = "button-borderless"
const BOX_TYPE_INLINE = "inline"

// Input Types
const INPUT_TYPE_MENU = "menu"
...
class UnityDropdown extends LitElement {
  ...
  filterSelection(selected) {
    ...
    return selected.reduce((newSelected, currentId) => {
      ...
      if (inputType === INPUT_TYPE_SINGLE_SELECT && newSelected.length === 1) return newSelected
  ...
  changeSelected(value) {
    this.selected = (this.inputType === INPUT_TYPE_SINGLE_SELECT) ? this.handleSingleSelect(value) : this.handleMultiSelect(value)
  }
  ...
  handleSingleSelect(id) {
    ...
    // Fall-through - maintain selection when clicking same element
          ...
          <div class="input-label-div${!showTags ? " no-tags" : ""} selectable" @click="${_dropdown}">
            <div class="displayed-wrapper">
              ...
              ${(showTags && anySelected) ? null : html`
                <p id="displayed" class=${(!anySelected && inputType !== INPUT_TYPE_MENU) ? "placeholder": ""}>
                  ${label || placeholder}
                </p>`}
            </div>
